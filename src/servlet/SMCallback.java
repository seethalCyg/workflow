 ...
 */
public class SMCallback extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(error != null && !error.isEmpty()) {
			...
			if(error.equalsIgnoreCase("invalid_client")) {
				errStr = "The client information is invalid. Ensure your client id is correct.";
			}else if(error.equalsIgnoreCase("invalid_grant")) {
				errStr = "The authorization code or refresh token is invalid or expired or the hash value does not match the app secret and/or code.";
			}else if(error.equalsIgnoreCase("invalid_request")) {
				errStr = "The request parameters are invalid or missing.";
			}else if(error.equalsIgnoreCase("invalid_scope")) {
				errStr = "One or more of the requested access scopes is invalid. Please check the list of access scopes.";
			}else if(error.equalsIgnoreCase("unsupported_grant_type")) {
				errStr = "grant_type must equal authorization_code or refresh_token.";
			}else if(error.equalsIgnoreCase("unsupported_response_type")) {
				errStr = "response_type must be set to code.";
			}else {
				errStr = "User has declined the request.";
			}
			response.getWriter().append("Smartsheet account is not authenticated. Because, "+errStr+" You can close this tab now.");
		...
		writer.close();
		response.getWriter().append("Smartsheet account is authenticated successfully. You can close this tab now.");
		}catch (Exception e) {
