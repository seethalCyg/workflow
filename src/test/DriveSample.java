...
public class DriveSample {
  ...
  private static final String UPLOAD_FILE_PATH = "C:\\Users\\Officepc\\Desktop\\Document Integration Collate documents.docx";
  private static final String DIR_FOR_DOWNLOADS = "C:\\Users\\Officepc\\Desktop\\console\\";
  ...
  private static Credential authorize() throws Exception {
    ...
    System.out.println("-=-=-=-=-=-=dfdf-=-=-=-="+DATA_STORE_DIR.getAbsolutePath());
...
//    flow.getCredentialDataStore().
    System.out.println("-=-=-=-=-=-getAccessType=-=-=-=-="+flow.getAccessType());
    System.out.println("-=-=-=-=-=-getClientId=-=-=-=-="+flow.getClientId());
    System.out.println("-=-=-=-=-=-getAuthorizationServerEncodedUrl=-=-=-=-="+new LocalServerReceiver.Builder().getCallbackPath());
    System.out.println("-=-=-=-=-=-getTokenServerEncodedUrl=-=-=-=-="+new LocalServerReceiver.Builder().getHost());
//    System.out.println("-=-=-=-=-=-getTokenServerEncodedUrl=-=-=-=-="+flow.get);
    // authorize
    return new AuthorizationCodeInstalledApp(flow, new LocalServerReceiver.Builder().setCallbackPath("/rbid/Callback").setPort(8080).build()).authorize("ppvishnu60@gmail.com");
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // authorization
      System.out.println("-=-=-=-=-ss=-=-=-=-=-="+DATA_STORE_DIR.getAbsolutePath());
      Credential credential = authorize();
      System.out.println("-=-=-=-=-=-sfde=-=-=-=-="+DATA_STORE_DIR.getAbsolutePath());
